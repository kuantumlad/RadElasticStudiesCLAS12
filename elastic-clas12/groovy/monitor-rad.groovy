    ...
    args.eachParallel { filename ->
	    ...
	    // Reconstructed (for data and simulation)
		    ...
		    def ctof = event.ctof_status.contains(it).findResult{ stat -> stat ? "CTOF" : "FTOF"}
                    ...
                    histos.computeIfAbsent('w_' + ctof + '_' + sector, histoBuilders.w_big).fill(pkin.w)
			...
			if (isr_fsr_flag != null && pass_missing_mass && pass_high_w && pass_angle_ep) {
			    ...
			    // Resolutions 
			    //trust pro P and Theta and Beam
				...
				if( ele.sphi > phi_low && ele.sphi <= phi_max ){
				    phi_bin=Integer.toString(pb)
				    //println(' event phi angle ' + ele.sphi + ' bin is ' + phi_bin )
				}
				phi_max=phi_low
				phi_low-=phi_range
			    }
			    ...
			    histos.computeIfAbsent('p_pro_dp_pro_' + ctof + '_' + sector + '_' + isr_fsr_flag, histoBuilders2.p_pro_dp).fill(pro.p(), pred_pro.momentum - pro.p())
			    histos.computeIfAbsent('p_pro_dp_pro_from_angles_' + ctof + '_' + sector + '_' + isr_fsr_flag, histoBuilders2.p_pro_dp).fill(
				pro.p(), pred_pro_p - pro.p())
			    histos.computeIfAbsent('dp_pro_from_angles_' + ctof + '_' + sector + '_thetabin' + theta_bin + '_' + isr_fsr_flag, histoBuilders.dp_pro).fill((pred_pro_p - pro.p())/pro.p())
			    histos.computeIfAbsent('dp_pro_from_angles_' + ctof + '_' + sector + '_thetabin' + theta_bin + '_phibin' + phi_bin + '_' + isr_fsr_flag, histoBuilders.dp_pro).fill((pred_pro_p - pro.p())/pro.p())
