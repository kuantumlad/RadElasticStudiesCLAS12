...
def h_theta_bin_elastic = new H1F("elastic_thetabins","elastic_thetabins", 7, 7.0, 35.0)
...
el_mntm_corr_par = ['1': el_s1_prftof,
		    '2': el_s2_prftof,
		    '3': el_s3_prftof,
		    '4': el_s4_prftof,
		    '5': el_s5_prftof,
...
//println( el_mntm_corr_par[4][2])
println(el_s1_prftof)
		...
		// Believe the electron angular measurement and infer the rest of the
			...
			//def pass_missing_mass = pkin.missing_mass > cuts.missing_mass[0] && pkin.missing_mass < cuts.missing_mass[1]
			if( pass_missing_mass && pass_high_w && pass_angle_ep){
			...
			if (isr_fsr_flag != null && pass_missing_mass && pass_high_w && pass_angle_ep) {
			    ...
			    if( isr_fsr_flag == 'isr' && ctof == 'FTOF' ){ 
				
				println(' sector ' + sector + ' theta bin ' + theta_bin_isr)
				def gdmfnumbers = el_mntm_corr_par[sector.toString()]
				def thetabins_to_get = gdmfnumbers
				println(thetabins_to_get[theta_bin_isr.toString()])

				def p_cor = momentumCorrection(delta_p, el_mntm_corr_par[sector.toString()][theta_bin_isr.toString()])
				println(' correction amount ' + p_cor )

				new_p = ele.p() + p_cor //momentumCorrection(delta_p, el_s1_prftof[theta_bin_isr])				
				...
				ele.p(), (pred_ele_p - ele.p())/ele.p())
			    //okay
 			    histos.computeIfAbsent('p_ele_dp_ele_from_angles_' + ctof + '_' + sector + '_thetabin' + theta_bin + '_phibin' + phi_bin + '_' + isr_fsr_flag, histoBuilders2.p_ele_dp).fill(
				ele.p(), (pred_ele_p - ele.p())/ele.p())
			    //okay
			...
			histos.computeIfAbsent('missing_mass_eep_rad_elastic_' + ctof, histoBuilders.missing_mass).fill(pkin.missing_mass)
		    }
		    if( pass_missing_mass && pass_high_w ){
			histos.computeIfAbsent('angle_ep_eep_rad_elastic_' + ctof, histoBuilders.angle_ep).fill(pkin.angle)
		    }
		    		    
		    //ISR
		    if (pass_angle_ep && pass_missing_mass && pass_theta_gamma){ //remove
		    ...
		    //FSR
		    if (pass_angle_ep && pass_missing_mass && pass_theta_egamma){
			histos.computeIfAbsent('w_eep_' + ctof + '_fsr', histoBuilders.w).fill(pkin.w)
		    }

 		    if (pass_missing_mass && pass_high_w && pass_theta_egamma){
			...
			histos.computeIfAbsent('missing_mass_eep_' + ctof  + '_elastic', histoBuilders.missing_mass).fill(pkin.missing_mass)
		    }
...
out.writeFile("mon-kcor-testing_correction.hipo")

def root_out = new ROOTFile("mon-kcor-testing_correction.root")
